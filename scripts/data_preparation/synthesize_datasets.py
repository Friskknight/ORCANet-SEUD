...
def generateTransforms_o(prob):
    all_transforms = [
                    AddGaussianNoise(10, 15), AddPoissonNoise(alpha=2, beta=4), AddSpeckleNoise(10, 15),
...
def generateTransforms(prob):
    base_transforms = [
        ...
        AddJPEGCompression([20, 30, 40]),
        AddVideoCompression(['libx264', 'h264', 'mpeg4']),
        AddGaussianBlur([3, 5, 7]),
        AddResizingBlur(["area", "bilinear", "bicubic"])
    ]

    transform_flags = [0] * len(base_transforms)
    selected_transforms = []
    selected_param_refs = {}

    for idx, t in enumerate(base_transforms):
        if random.random() > prob:
            selected_transforms.append(t)
            transform_flags[idx] = 1
            selected_param_refs[type(t).__name__] = t  # 保留用于后续提参数

    return transforms.Compose(selected_transforms), transform_flags, selected_param_refs
...
def main(input_folder, output_folder, continuous_frames=6, prob=0.55):
    ...
    for folder in sub_folders:
        ...
        deg_transform, deg_label, deg_param_objs = generateTransforms(prob)
        label_dict = {}
        ...
        for frame_name in sorted(os.listdir(input_sub_folder)):
            ...
            applied_params = {}
            for name, t in deg_param_objs.items():
                applied_params[name] = t.__dict__

            label_dict[frame_name] = {
                "applied": deg_label.copy(),
        ...
        label_json_path = os.path.join(output_sub_folder, "degradation_labels.json")
