...
class VideoRecurrentModel(VideoBaseModel):
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for i in range(rank, num_folders + num_pad, world_size):
            ...
            val_data = dataset[idx]
            folder = val_data['folder']
            base = val_data.get('start_idx', 0)  # NEW/CHANGED: 该 chunk 在该 folder 内的起始帧索引
            ...
            # #######
            # sorted_keys = sorted(self.static_p.keys())
            # static_p_list = [self.static_p[k].squeeze(0).cpu().numpy() for k in sorted_keys]
            # frame_ids_with_folder = [f"{folder}_{str(k).zfill(5)}" for k in sorted_keys]
            # all_frame_ids.extend(frame_ids_with_folder)
            # all_static_p.extend(static_p_list)
            # all_cat.extend(self.valdeg)
            # print(len(static_p_list))
            # print(len(self.valdeg))
            # del self.static_p
            # #######
            ...
            # evaluate
            if i < num_folders:
                # 注意：这里的“idx”已用于选择数据条目，避免重名，帧内循环用 t
                for t in range(visuals['result'].size(1)):
                    result = visuals['result'][0, t, :, :, :]
                    metric_data['img'] = self.metric_cal_pre(result)
                    result_img = tensor2img([result])  # uint8, bgr
                    ...
                    if save_img:
                        ...
                        # if False:
                            ...
                            if self.center_frame_only:  # vimeo-90k
                                ...
                                global_idx = base + t  # NEW/CHANGED: 使用全局帧号，避免跨 chunk 覆盖
                                img_path = osp.join(self.opt['path']['visualization'], dataset_name, folder,
                                                    f"{global_idx:08d}_{self.opt['name']}.png")
                        imwrite(result_img, img_path)

                    # calculate metrics
                    if with_metrics:
                        global_idx = base + t  # NEW/CHANGED: 指标写入使用全局帧号
                        for metric_idx, opt_ in enumerate(self.opt['val']['metrics'].values()):
                            iqa_model = create_metric(opt_['type'], metric_mode='FR')
                            result_score = iqa_model(metric_data['img'], metric_data['img2']).cpu().item()

                            # NEW/CHANGED: 直接赋值到该帧，而不是+=（避免同一帧被多个chunk重复累加）
                            self.metric_results[folder][global_idx, metric_idx] = result_score

                # progress bar
                if rank == 0:
                    for _ in range(world_size):
                        pbar.update(1)
                        # NEW/CHANGED: 显示该chunk起点，便于定位
                        pbar.set_description(f'Folder: {folder} | start={base}')
            val_data['lq'].squeeze_(0)
            # val_data['dp'].squeeze_(0)
            val_data['gt'].squeeze_(0)

        # #######
        # prompts_cat = {
        #     "frame_ids": np.array(all_frame_ids, dtype=object),
        #     "static_p": np.array(all_static_p),   # shape (total_N, 5)
        #     "cat": np.array(all_cat)              # shape (total_N, 3)
        # }
        # np.save(osp.join(self.opt['path']['log'], "prompt_static_and_cat.npy"), prompts_cat)
        # print(f'save npy')
        # metric_results = self.metric_results  # 你的字典
        # metric_results = {k: (v.detach().cpu().tolist() if isinstance(v, torch.Tensor) else v)
        #                 for k, v in metric_results.items()}

        # with open(osp.join(self.opt['path']['log'], "metric_results.json"), "w") as f:
    ...
    def test(self):
        ...
        if patch_testing_size:
            ...
            with torch.no_grad():
                ...
                # self.output, self.static_p = self.net_g(self.lq)
