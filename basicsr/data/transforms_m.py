...
def paired_random_crop(img_gts, img_depths, img_lqs, gt_patch_size, scale, gt_path=None):
    ...
    Args:
        ...
        img_depths (list[ndarray] | ndarray | list[Tensor] | Tensor): Depth images. Note that all images
            should have the same shape. If the input is an ndarray, it will
            be transformed to a list containing itself.
    ...
    if not isinstance(img_gts, list):
        img_gts = [img_gts]
    if not isinstance(img_depths, list):
        img_depths = [img_depths]
    ...
    if input_type == 'Tensor':
        ...
        h_dp, w_dp = img_depths[0].size()[-2:]
        h_gt, w_gt = img_gts[0].size()[-2:]
    else:
        h_lq, w_lq = img_lqs[0].shape[0:2]
        h_dp, w_dp = img_depths[0].shape[0:2]
    ...
    if input_type == 'Tensor':
        ...
        img_depths = [w[:, :, top:top + lq_patch_size, left:left + lq_patch_size] for w in img_depths]
    else:
        img_lqs = [v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_lqs]
        img_depths = [v[top:top + lq_patch_size, left:left + lq_patch_size, ...] for v in img_depths]
    ...
    if len(img_gts) == 1:
        img_gts = img_gts[0]
        img_depths = img_depths[0]
    if len(img_lqs) == 1:
        img_lqs = img_lqs[0]
    return img_gts, img_depths, img_lqs
