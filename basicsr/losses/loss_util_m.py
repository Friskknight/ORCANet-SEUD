...
def weighted_loss(loss_func):
    ...
    **kwargs)`.
    ...
    # def wrapper(pred, target, um, deg_app, weight=None, reduction='mean', **kwargs):
    #     # get element-wise loss
    #     loss = loss_func(pred, target, **kwargs)    # torch.Size([2, 12, 3, 256, 256])
    ...
    def wrapper(pred, target, um, deg_app, weight=None, reduction='mean', **kwargs):
        ...
        # 计算元素级 Charbonnier loss
        pixel_loss = loss_func(pred, target, **kwargs)  # [B, T, C, H, W]
        pixel_loss = pixel_loss.unsqueeze(2)  # [B, T, 1, C, H, W]

        # 按任务加权
        # um: [B, T, K, C, H, W]，代表每种退化的 σ_k（正数）
        # 按公式 ∑_k (1 / (2σ_k^2)) * loss + 1/2 log(σ_k^2)
        um = um.view(B, T, K, C, H, W)      # (2, 12, 7, 3, 256, 256)
        weighted_loss = (1.0 / (2.0 * um ** 2)) * pixel_loss + 0.001 * torch.log(um ** 2 + 1e-6)

        # Step 4: 按 deg_app 进行 mask，仅加权存在的退化类型
        deg_app = deg_app.view(B, T, K, 1, 1, 1)  # broadcast to um shape
        masked_loss = weighted_loss * deg_app  # only keep active degradations

        # Step 5: 合并任务维度
        total_loss = masked_loss.sum(dim=2)  # [B, T, C, H, W]

        # Step 6: reduce
        final_loss = weight_reduce_loss(total_loss, weight, reduction)
        return final_loss
